...
const MachineCard = ({ machine }) => {
  ...
  const changePercent = (machine?.oee || 0) - (machine?.oeeYesterday || 0); // So sánh với ngày hôm qua
  const changeIcon = changePercent >= 0 ? <FaArrowUp className="text-green-700 inline" /> : <FaArrowDown className="text-blue-950 inline" />;

  const timeChangePercent = ((machine?.totalTimeToday || 0 - (machine?.totalTimeYesterday || 1)) / (machine?.totalTimeYesterday || 1)) * 100;
    ...
    <div className={`shadow-md flex flex-col justify-between h-full ${blinkClass}`} style={{ backgroundColor: headerColor }}>
      ...
      <div className="flex items-center ml-2 justify-center bg-transparent p-2 mb-8">
        ...
        <div className="relative ml-2" style={{ width: 200, height: 200 }}>
          <CircularProgressbar
            value={(machine?.summaryStatus || 0) / 86400 * 100}
            styles={buildStyles({
              pathColor: '#0782f4',
              textColor: '#122a35',
              fontSize: 'bold',
